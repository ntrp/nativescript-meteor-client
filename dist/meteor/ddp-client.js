...
_.extend(LivedataTest.ClientStream.prototype, {                                                                      // 32
  ...
  send: function (data) {                                                                                            // 37
    ...
    if (self.currentStatus.connected) {                                                                              // 39
      if (__meteor_runtime_config__.nsDebug) {                                                                       // 40
        Meteor._debug("ws --> ", data);                                                                              // 41
      }                                                                                                              // 42
      self.client.send(data);                                                                                        // 43
    }                                                                                                                // 44
  },                                                                                                                 // 45
                                                                                                                     // 46
    ...
    if (self.client) {                                                                                               // 98
      ...
      _.each(self.eventCallbacks.disconnect, function (callback) {                                                   // 101
        callback(maybeError);                                                                                        // 102
                                                                                                                     ...
                                                                                                                     // 106
  _clearConnectionTimer: function () {                                                                               // 107
    var self = this;                                                                                                 // 108
                                                                                                                     // 109
    if (self.connectionTimer) {                                                                                      // 110
      ...
      self.connectionTimer = null;                                                                                   // 112
                                                                                                                     ...
                                                                                                                     // 115
  _getProxyUrl: function (targetUrl) {                                                                               // 116
    var self = this;                                                                                                 // 117
    // Similar to code in tools/http-helpers.js.                                                                     // 118
    var proxy = process.env.HTTP_PROXY || process.env.http_proxy || null;                                            // 119
    // if we're going to a secure url, try the https_proxy env variable first.                                       // 120
    ...
    return proxy;                                                                                                    // 124
                                                                                                                     ...
                                                                                                                     // 126
  _launchConnection: function () {                                                                                   // 127
    var self = this;                                                                                                 // 128
    ...
    // connection.                                                                                                   // 133
    var NativeWebSocket = require('nativescript-websockets');                                                    // 134
                                                                                                                     // 135
    var targetUrl = toWebsocketUrl(self.endpoint);                                                                   // 136
                                                                                                                     // 137
    // We would like to specify 'ddp' as the subprotocol here. The npm module we                                     // 138
    // used to use as a client would fail the handshake if we ask for a                                              // 139
    // subprotocol and the server doesn't send one back (and sockjs doesn't).                                        // 140
    // Faye doesn't have that behavior; it's unclear from reading RFC 6455 if                                        // 141
    // Faye is erroneous or not.  So for now, we don't specify protocols.                                            // 142
    var subprotocols = [];                                                                                           // 143
                                                                                                                     // 144
    var client = self.client = new NativeWebSocket(targetUrl, {                                                      // 145
        timeout: __meteor_runtime_config__.nsSocketTimeout || 6000,                                                  // 146
        allowCellular: true                                                                                          // 147
    });                                                                                                              // 148
                                                                                                                     // 149
    self._clearConnectionTimer();                                                                                    // 150
    self.connectionTimer = Meteor.setTimeout(                                                                        // 151
      function () {                                                                                                  // 152
        self._lostConnection(                                                                                        // 153
          new DDP.ConnectionError("DDP connection timed out"));                                                      // 154
      },                                                                                                             // 155
      self.CONNECT_TIMEOUT);                                                                                         // 156
                                                                                                                     // 157
    self.client.on('open', Meteor.bindEnvironment(function () {                                                      // 158
      return self._onConnect(client);                                                                                // 159
    }, "stream connect callback"));                                                                                  // 160
                                                                                                                     // 161
    var clientOnIfCurrent = function (event, description, f) {                                                       // 162
      self.client.on(event, Meteor.bindEnvironment(function () {                                                     // 163
        // Ignore events from any connection we've already cleaned up.                                               // 164
        if (client !== self.client)                                                                                  // 165
          return;                                                                                                    // 166
        f.apply(this, arguments);                                                                                    // 167
      }, description));                                                                                              // 168
    };                                                                                                               // 169
                                                                                                                     // 170
    clientOnIfCurrent('error', 'stream error callback', function (error) {                                           // 171
      if (!self.options._dontPrintErrors)                                                                            // 172
        Meteor._debug("stream error", error.message);                                                                // 173
                                                                                                                     // 174
      // Faye's 'error' object is not a JS error (and among other things,                                            // 175
      // doesn't stringify well). Convert it to one.                                                                 // 176
      self._lostConnection(new DDP.ConnectionError(error.message));                                                  // 177
    });                                                                                                              // 178
                                                                                                                     // 179
    clientOnIfCurrent('close', 'stream close callback', function () {                                                // 180
      self._lostConnection();                                                                                        // 181
    });                                                                                                              // 182
                                                                                                                     // 183
    clientOnIfCurrent('message', 'stream message callback', function (socket, message) {                             // 184
      // Ignore binary frames, where message.data is a Buffer                                                        // 185
      if (typeof message !== "string")                                                                               // 186
        return;                                                                                                      // 187
                                                                                                                     // 188
      if (__meteor_runtime_config__.nsDebug) {                                                                       // 189
          Meteor._debug("ws <-- ", message);                                                                         // 190
      }                                                                                                              // 191
                                                                                                                     // 192
      _.each(self.eventCallbacks.message, function (callback) {                                                      // 193
        callback(message);                                                                                           // 194
      });                                                                                                            // 195
    });                                                                                                              // 196
    // open connection                                                                                               // 197
    self.client.open();                                                                                              // 198
  }                                                                                                                  // 199
});                                                                                                                  // 200
                                                                                                                     // 201
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
