...
var require = meteorInstall({"node_modules":{"meteor":{"ddp-client":{"stream_client_nativescript.js":["babel-runtime/helpers/classCallCheck","./namespace.js",function(require,exports,module){
...
LivedataTest.ClientStream = function () {                                                                              // 14
    ...
    function send(data) {                                                                                              // 14
      ...
      if (self.currentStatus.connected) {                                                                              // 40
        self.client.send(data);                                                                                        // 41
                                                                                                                       //
        if (__meteor_runtime_config__.nsDebug) {                                                                       // 42
          Meteor._debug("ws --> ", data);                                                                              // 43
        }                                                                                                              // 44
      }                                                                                                                // 45
    }                                                                                                                  // 46
                                                                                                                       //
  ...
  ClientStream.prototype._onConnect = function () {                                                                    // 14
    function _onConnect(client) {                                                                                      // 14
      var self = this;                                                                                                 // 55
                                                                                                                       //
      if (client !== self.client) {                                                                                    // 57
        // This connection is not from the last call to _launchConnection.                                             // 58
        // But _launchConnection calls _cleanup which closes previous connections.                                     // 59
        // It's our belief that this stifles future 'open' events, but maybe                                           // 60
        // we are wrong?                                                                                               // 61
        throw new Error("Got open from inactive client " + !!self.client);                                             // 62
      }                                                                                                                // 63
                                                                                                                       //
      if (self._forcedToDisconnect) {                                                                                  // 65
        // We were asked to disconnect between trying to open the connection and                                       // 66
        // actually opening it. Let's just pretend this never happened.                                                // 67
        self.client.close();                                                                                           // 68
        self.client = null;                                                                                            // 69
        return;                                                                                                        // 70
      }                                                                                                                // 71
                                                                                                                       //
      if (self.currentStatus.connected) {                                                                              // 73
        // We already have a connection. It must have been the case that we                                            // 74
        // started two parallel connection attempts (because we wanted to                                              // 75
        // 'reconnect now' on a hanging connection and we had no way to cancel the                                     // 76
        // connection attempt.) But this shouldn't happen (similarly to the client                                     // 77
        // !== self.client check above).                                                                               // 78
        throw new Error("Two parallel connections?");                                                                  // 79
      }                                                                                                                // 80
                                                                                                                       //
      self._clearConnectionTimer(); // update status                                                                   // 82
                                                                                                                       //
                                                                                                                       //
      self.currentStatus.status = "connected";                                                                         // 85
      self.currentStatus.connected = true;                                                                             // 86
      self.currentStatus.retryCount = 0;                                                                               // 87
      self.statusChanged(); // fire resets. This must come after status change so that clients                         // 88
      // can call send from within a reset callback.                                                                   // 91
                                                                                                                       //
      _.each(self.eventCallbacks.reset, function (callback) {                                                          // 92
        callback();                                                                                                    // 92
      });                                                                                                              // 92
    }                                                                                                                  // 93
                                                                                                                       //
  ...
  ClientStream.prototype._cleanup = function () {                                                                      // 14
    function _cleanup(maybeError) {                                                                                    // 14
      var self = this;                                                                                                 // 96
                                                                                                                       //
      self._clearConnectionTimer();                                                                                    // 98
                                                                                                                       //
      if (self.client) {                                                                                               // 99
        var client = self.client;                                                                                      // 100
        self.client = null;                                                                                            // 101
        client.close();                                                                                                // 102
                                                                                                                       //
        _.each(self.eventCallbacks.disconnect, function (callback) {                                                   // 104
          callback(maybeError);                                                                                        // 105
        });                                                                                                            // 106
      }                                                                                                                // 107
    }                                                                                                                  // 108
                                                                                                                       //
  ...
  ClientStream.prototype._clearConnectionTimer = function () {                                                         // 14
    function _clearConnectionTimer() {                                                                                 // 14
      var self = this;                                                                                                 // 111
                                                                                                                       //
      if (self.connectionTimer) {                                                                                      // 113
        clearTimeout(self.connectionTimer);                                                                            // 114
        self.connectionTimer = null;                                                                                   // 115
      }                                                                                                                // 116
    }                                                                                                                  // 117
                                                                                                                       //
  ...
  ClientStream.prototype._getProxyUrl = function () {                                                                  // 14
    function _getProxyUrl(targetUrl) {                                                                                 // 14
      var self = this; // Similar to code in tools/http-helpers.js.                                                    // 120
                                                                                                                       //
      var proxy = process.env.HTTP_PROXY || process.env.http_proxy || null; // if we're going to a secure url, try the https_proxy env variable first.
                                                                                                                       //
      if (targetUrl.match(/^wss:/)) {                                                                                  // 124
        proxy = process.env.HTTPS_PROXY || process.env.https_proxy || proxy;                                           // 125
      }                                                                                                                // 126
                                                                                                                       //
      return proxy;                                                                                                    // 127
    }                                                                                                                  // 128
  ...
  ClientStream.prototype._launchConnection = function () {                                                             // 14
    function _launchConnection() {                                                                                     // 14
      var self = this;                                                                                                 // 131
                                                                                                                       //
      self._cleanup(); // cleanup the old socket, if there was one.                                                    // 132
      // Since server-to-server DDP is still an experimental feature, we only                                          // 134
      // require the module if we actually create a server-to-server                                                   // 135
      // connection.                                                                                                   // 136
                                                                                                                       //
                                                                                                                       //
      ...
      // subprotocol and the server doesn't send one back (and sockjs doesn't).                                        // 142
      // Faye doesn't have that behavior; it's unclear from reading RFC 6455 if                                        // 143
      // Faye is erroneous or not.  So for now, we don't specify protocols.                                            // 144
                                                                                                                       //
      var subprotocols = [];                                                                                           // 145
      var client = self.client = new NativeWebSocket(targetUrl, {                                                      // 147
        timeout: __meteor_runtime_config__.nsSocketTimeout || 6000,                                                    // 148
        allowCellular: true                                                                                            // 149
      });                                                                                                              // 147
                                                                                                                       //
      self._clearConnectionTimer();                                                                                    // 152
                                                                                                                       //
      self.connectionTimer = Meteor.setTimeout(function () {                                                           // 153
        self._lostConnection(new DDP.ConnectionError("DDP connection timed out"));                                     // 155
      }, self.CONNECT_TIMEOUT);                                                                                        // 157
      self.client.on('open', Meteor.bindEnvironment(function () {                                                      // 160
        return self._onConnect(client);                                                                                // 161
      }, "stream connect callback"));                                                                                  // 162
                                                                                                                       //
      var clientOnIfCurrent = function (event, description, f) {                                                       // 164
        self.client.on(event, Meteor.bindEnvironment(function () {                                                     // 165
          // Ignore events from any connection we've already cleaned up.                                               // 166
          if (client !== self.client) return;                                                                          // 167
          f.apply(this, arguments);                                                                                    // 169
        }, description));                                                                                              // 170
      };                                                                                                               // 171
                                                                                                                       //
      clientOnIfCurrent('error', 'stream error callback', function (error) {                                           // 173
        ...
        // doesn't stringify well). Convert it to one.                                                                 // 178
                                                                                                                       //
        self._lostConnection(new DDP.ConnectionError(error.message));                                                  // 179
      });                                                                                                              // 180
...
require("./node_modules/meteor/ddp-client/stream_client_nativescript.js");
